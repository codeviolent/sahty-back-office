...
import '../bloc/profile_panel_bloc.dart';
import '../bloc/profile_panel_event.dart';
import '../bloc/profile_panel_state.dart';
...
class _TopProfileButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ProfilePanelBloc, ProfilePanelState>(
      builder: (ctx, state) {
        final enabled = onPressed != null;
        final isFrench = context.appLocale == AppLocale.fr;
        final profileName = state is ProfilePanelLoaded
            ? state.profile.displayName
            : '...';
        final initials = state is ProfilePanelLoaded
            ? state.profile.initials
...
// Remplacer _RightContextPanel.build() entièrement
...
class _RightContextPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: BlocBuilder<ProfilePanelBloc, ProfilePanelState>(
        builder: (ctx, state) {
          if (state is ProfilePanelLoading || state is ProfilePanelInitial) {
            return const Center(
          ...
          if (state is ProfilePanelError) {
            return _PanelError(
              message: state.message,
              onRetry: () => ctx.read<ProfilePanelBloc>().add(
                ProfilePanelRefreshRequested(),
              ),
            );
          }
          if (state is ProfilePanelLoaded) {
            return _PanelContent(profile: state.profile);
          }
...
// ── Contenu du panel selon le rôle ────────────────────────────────
class _PanelContent extends StatelessWidget {
  final UserProfile profile;
  const _PanelContent({required this.profile});
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Stats selon le rôle ───────────────────────────────
          _ProfileStatsSection(profile: profile),
          const SizedBox(height: AppSpacing.lg),
          const Divider(),
          const SizedBox(height: AppSpacing.md),

          // ── Agenda du jour (médecin + staff) ──────────────────
          if (profile.isMedecin || profile.isStaff) ...[
            _PanelLabel(title: context.tr(AppTextKey.myCalendar)),
            const SizedBox(height: AppSpacing.md),
            _CalendarStrip(),
            const SizedBox(height: AppSpacing.md),
            _AgendaSection(profile: profile),
            const SizedBox(height: AppSpacing.lg),
            const Divider(),
            const SizedBox(height: AppSpacing.md),
          ],
          ...
          _AlertesSection(profile: profile),
        ],
      ),
    );
  }
...
class _ProfileStatsSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Construire les 3 cellules selon le rôle
    final cells = _buildCells(context);
        ...
        child: Row(
          ...
          children: [
            for (int i = 0; i < cells.length; i++) ...[
              Expanded(
                child: _StatCell(label: cells[i].$1, value: cells[i].$2),
              ),
              if (i < cells.length - 1)
  ...
  List<(String, String)> _buildCells(BuildContext context) {
    final s = profile.stats;

    if (profile.isMedecin) {
      return [
        (
          context.tr(AppTextKey.doctorDepartment),
          s.department.isNotEmpty ? s.department : '--',
        ),
        (context.tr(AppTextKey.doctorAvailability), s.isAvailable ? '✓' : '✗'),
        (context.tr(AppTextKey.workingHours), s.workingHours),
      ];
    }

    if (profile.isAdmin) {
      return [
        ('Patients', s.totalPatients.toString()),
        ('Médecins', s.totalDoctors.toString()),
        ('RDV/jour', s.rdvToday.toString()),
      ];
    }

    if (profile.isStaff) {
      return [
        ('RDV/jour', s.rdvTotal.toString()),
        ('En attente', s.rdvPending.toString()),
        ('Confirmés', (s.rdvTotal - s.rdvPending).toString()),
      ];
    }

    if (profile.isPharma) {
      return [
        ('Dispensés', s.dispensationsToday.toString()),
        ('Actives', s.prescriptionsActive.toString()),
        ('Statut', '✓'),
      ];
    }
...
class _AgendaSection extends StatelessWidget {
  final UserProfile profile;
  const _AgendaSection({required this.profile});

  @override
  Widget build(BuildContext context) {
    final agenda = profile.todayAgenda;
    ...
    return Column(
      children: agenda.map((rdv) {
        final (label, tone) = switch (rdv.status) {
          'confirmed' => ('Confirmé', BadgeTone.normal),
          'critical' => ('Critique', BadgeTone.critical),
          'teleconsult' => ('En ligne', BadgeTone.info),
          'completed' => ('Terminé', BadgeTone.neutral),
          _ => ('En attente', BadgeTone.neutral),
        };
        ...
        return Padding(
          ...
          child: _ContextItem(
            time: rdv.time,
            title: rdv.patientName.isNotEmpty
                ? '${rdv.patientName}${rdv.reason.isNotEmpty ? " — ${rdv.reason}" : ""}'
                : rdv.reason.isNotEmpty
                ? rdv.reason
                : 'Consultation',
            label: label,
            tone: tone,
...
class _AlertesSection extends StatelessWidget {
  final UserProfile profile;
  const _AlertesSection({required this.profile});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // Alerte selon le rôle
        if (profile.isMedecin) ...[
          if (profile.unreadMessages > 0) ...[
            const SizedBox(height: AppSpacing.sm),
            StatusBadge(
              label: '${profile.unreadMessages} message(s) non lu(s)',
              tone: BadgeTone.info,
        ...
        if (profile.isAdmin) ...[
          const SizedBox(height: AppSpacing.sm),
          StatusBadge(
            label: 'Accès administrateur complet',
            tone: BadgeTone.critical,
            icon: Icons.admin_panel_settings_outlined,
          ),
        ],

        if (profile.isStaff && profile.stats.rdvPending > 0) ...[
          const SizedBox(height: AppSpacing.sm),
          StatusBadge(
            label: '${profile.stats.rdvPending} RDV à confirmer',
            tone: BadgeTone.critical,
        ...
        if (profile.isPharma) ...[
          const SizedBox(height: AppSpacing.sm),
          StatusBadge(
            label: '${profile.stats.prescriptionsActive} ordonnances actives',
            tone: BadgeTone.info,
...
class _CalendarStrip extends StatelessWidget {
  ...
  static List<DateTime> _currentWeekDays() {
    ...
    // DateTime.weekday : Lun=1 ... Sam=6 Dim=7
    // On veut Dim=0, Lun=1, Mar=2 ... Sam=6
    // Décalage pour commencer la semaine le Dimanche
    final sundayOffset = today.weekday % 7; // Lun(1)→1, Dim(7)→0
    final sunday = today.subtract(Duration(days: sundayOffset));
        ...
        for (var i = 0; i < days.length; i++) ...[
          Expanded(
            child: _CalendarDay(
              ...
              date: week[i],
              isToday:
                  week[i].day == today.day &&
                  week[i].month == today.month &&
                  week[i].year == today.year,
            ),
          ),
        ],
      ],
      ...
      child: Column(
        children: [
          Text(
            dayLabel,
            style: TextStyle(
              color: isToday ? Colors.white : AppColors.placeholder,
              fontSize: 8.5,
              fontWeight: FontWeight.w600,
            ),
          ),
